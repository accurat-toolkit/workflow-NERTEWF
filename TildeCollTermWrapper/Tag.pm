#!/usr/bin/perl
#===========File: Tag.pm===============
#Title:        Tag.pm - text POS-tagging Module.
#Description:  The Module provides POS-tagging functionality of plaintext documents.
...
#=========Method: TagText==========
#Title:        TagText
#Description:  POS-tags a document (parameter 2) using a specified POS tagger (parameter 0) and language code (parameter 1). The results are saved in a data file (parameter 3). Deletes temp files if argument 4 exists and true, and removes empty lines if specified (argument 5).
#Author:       Kârlis Gediòð, SIA Tilde.
#Created:      May, 2011.
#Last Changes: 03.08.2011. by Mârcis Pinnis, SIA Tilde.
#===============================================
sub TagText 
{
	...
	if ($uTagger eq "TREE") #Tree tagger option.
	{
		...
		TEPreprocess::FindTokenPos("$_[2]" ,"$outputDir$filename.Tree","$outputDir$filename.temp"); 
	}	 
	...
	{
		TEPreprocess::RemoveEmptyLines("$outputDir$filename.temp","$_[3]",$_[5]);
	}
	else
	{
		TEPreprocess::RemoveEmptyLines("$outputDir$filename.temp","$_[3]",1);
	}
